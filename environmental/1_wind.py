...
for i in range (end+1):
    wind = [] #List of final wind speeds for each grid for the day
    agg = [] #List of aggregated wind speeds, for imputing missing values
    ...
    for pos in index_cd:
        if cd_actual.mask[pos[0],pos[1]] == False:
            wind.append(cd_actual[pos[0],pos[1]])
            agg.append('Actual')
        else:
            week = []
            pond=[]
    ...
    with open(output_file, 'a', newline="", encoding='UTF8') as f:
        writer = csv.writer(f)
        for c,p,a in zip(wind,points,agg):
